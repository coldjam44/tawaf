...
<div class="row">
    <div class="col-xl">
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h3 class="text-center my-4"> {{ trans('main_trans.hotelinmakkah') }}</h3>
                <div class="text-right mb-3">
                    @can('create apartments')
                    <a href="{{ route('hotelinmakkahs.create') }}" class="btn-bid-now" style="color:white; cursor:pointer">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered data-table" id="data-table">
                        ...
                        <tbody>
                            ...
                            @foreach ($hotelinmakkahs as $hotelinmakkah)

                                <tr>
                                    <td>{{ $hotelinmakkah->id }}</td>
                                    <td>@if(App::getLocale() == 'ar')
                                        {{ $hotelinmakkah->title_ar }}
                                    @else
                                        {{ $hotelinmakkah->title_en }}
                                    @endif</td>
                                    <td>@if(App::getLocale() == 'ar')
                                        {{ $hotelinmakkah->description_ar }}
                                    @else
                                        {{ $hotelinmakkah->description_en }}
                                    @endif</td>

                                    {{-- <td>{{ App::getLocale() == 'ar' ? $hotelinmakkah->hotel->name_ar : $hotelinmakkah->hotel->name_en }}</td> --}}

                                    <td>
                                        @if ($hotelinmakkah->hotels->isNotEmpty())
                                            {{ $hotelinmakkah->hotels->map(fn($hotel) => App::getLocale() == 'ar' ? $hotel->name_ar : $hotel->name_en)->implode(' , ') }}
                                        @else
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-info btn-sm" data-toggle="modal"
                                            data-target="#edit{{ $hotelinmakkah->id }}"
                                            title="{{ trans('Counters_trans.Edit') }}"><i
                                                class="fa fa-edit"></i></button>
                                        <button type="button" class="btn btn-danger btn-sm" data-toggle="modal"
                                            data-target="#delete{{ $hotelinmakkah->id }}"
                                    ...
                                    <!-- edit_modal_Grade -->
                                    <div class="modal fade" id="edit{{ $hotelinmakkah->id }}" tabindex="-1" role="dialog"
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <!-- add_form -->
                                                    <form action="{{ route('hotelinmakkahs.update',$hotelinmakkah->id) }}" method="post" enctype="multipart/form-data">
                                                        {{ method_field('patch') }}
                                                        @csrf
                                                        <div class="row">
                                                            <div class="col">
                                                                ...
                                                                <input id="title_ar" type="text" name="title_ar"
                                                                    class="form-control"
                                                                    value="{{ $hotelinmakkah->title_ar }}"
                                                                    required>
                                                                <input id="id" type="hidden" name="id"
                                                                    class="form-control" value="{{ $hotelinmakkah->id }}">
                                                            </div>
                                                            <div class="col">
                                                                <label for="title_en"
                                                        ...
                                                        <div class="row">
                                                            <div class="col">
                                                                ...
                                                                <input id="description_ar" type="text" name="description_ar"
                                                                    class="form-control"
                                                                    value="{{ $hotelinmakkah->description_ar }}"
                                                                    required>
                                                                <input id="id" type="hidden" name="id"
                                                                    class="form-control" value="{{ $hotelinmakkah->id }}">
                                                            </div>
                                                            <div class="col">
                                                                <label for="description_en"
                                                                    ...
                                                                    :</label>
                                                                <input type="text" class="form-control"
                                                                    value="{{ $hotelinmakkah->description_en }}"
                                                                    ...
                                                                    <li>
                                                                        <label class="dropdown-item">
                                                                            <input
                                                                                ...
                                                                                name="hotel_ids[]"
                                                                                value="{{ $category->id }}"
                                                                                {{ $hotelinmakkah->hotels->contains($category->id) ? 'checked' : '' }}>
                                                            ...
                                                            <select name="hotel_id[]" class="form-control form-select" id="amenity-select" multiple>
                                                                @foreach ($hotels as $category)
                                                                    <option value="{{ $category->id }}"
                                                                            {{ in_array($category->id, old('hotel_id', $hotelinmakkah->hotel->pluck('id')->toArray())) ? 'selected' : '' }}>
                                                                        {{ App::getLocale() === 'ar' ? $category->name_ar : $category->name_en }}
                                                                    </option>
                                                                @endforeach
                                                            </select>
    ...
    {{-- //////////////////////////////////// --}}
                                    <!-- delete_modal_Grade -->
                                    <div class="modal fade" id="delete{{ $hotelinmakkah->id }}" tabindex="-1" role="dialog"
                                        aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                ...
                                                <div class="modal-body">
                                                    <form action="{{route('hotelinmakkahs.destroy',$hotelinmakkah->id)}}" method="post">
                                                        {{method_field('Delete')}}
                                                        @csrf
                                                        {{ trans('Counters_trans.Warning_Grade') }}
                                                        <input id="id" type="hidden" name="id" class="form-control"
                                                               value="{{ $hotelinmakkah->id }}">
                                                        <div class="modal-footer">
                                                            <button type="button" class="btn btn-secondary"
                                                                    data-dismiss="modal">{{ trans('Counters_trans.Close') }}</button>
                                                            <button type="submit"
                        ...
                        {{ $hotelinmakkahs->links('pagination::bootstrap-5') }}
        ...
        <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <!-- add_form -->
                        <form action="{{ route('hotelinmakkahs.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                <div class="col">
...
<br><br>
                            ...
                            {{-- <div class="form-group dropdown">
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                    @foreach ($hotels as $category)
                                        @if ($category->possition_en === 'Makkah' || $category->possition_ar === 'مكة')
                                            <li>
                                                <label class="dropdown-item">
                                                    <input type="checkbox" name="hotel_ids[]" value="{{ $category->id }}">
                                                    {{ App::getLocale() === 'ar' ? $category->name_ar : $category->name_en }}
