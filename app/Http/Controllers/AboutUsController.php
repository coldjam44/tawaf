...
class AboutUsController extends Controller
{
    ...
    {
        $validator = Validator::make($request->all(), [
            'section_name' => 'required|string|max:255|unique:about_us_sections,section_name,' . $id,
            'title_ar' => 'required|string|max:255',
            'title_en' => 'required|string|max:255',
            'content_ar' => 'required|string',
            'content_en' => 'required|string',
        ...
        if ($validator->fails()) {
            \Log::error('AboutUs Update Validation Errors: ' . json_encode($validator->errors()));
            return redirect()->back()->withErrors($validator)->withInput();
        }
        ...
        try {
            ...
            // Handle main image upload
            $mainImageName = $section->main_image; // Keep existing image if no new one uploaded
            if ($request->hasFile('main_image')) {
                // Delete old main image
                if ($section->main_image) {
                    $oldImagePath = public_path('about-us/main-images/' . $section->main_image);
            ...
            return redirect()->route('about-us.index')
                           ->with('success', trans('main_trans.about_us_section_updated_successfully'));

        } catch (\Exception $e) {
            \Log::error('AboutUs Update Error: ' . $e->getMessage());
            \Log::error('Stack trace: ' . $e->getTraceAsString());
            return redirect()->back()->with('error', 'An error occurred while updating the about us section: ' . $e->getMessage())->withInput();
        }
    }
    ...
    public function show()
    {
        ...
        return view('about-us.show', compact('sections'));
    }
