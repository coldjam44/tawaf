...
class AbuDhabiAreasRealImagesSeeder extends Seeder
{
    /**
    ...
    {
        // Define areas with their real image URLs from official sources
        $areas = [
            [
            ...
            [
                ...
                'name_en' => 'Zayed City',
                'slug' => 'zayed-city',
                'image_url' => 'https://www.visitabudhabi.ae/-/media/visitabudhabi/plan-your-trip/where-to-stay/abu-dhabi-city/abu-dhabi-city-hero.jpg',
            ...
            [
                ...
                'name_en' => 'Masdar City',
                'slug' => 'masdar-city',
                'image_url' => 'https://www.visitabudhabi.ae/-/media/visitabudhabi/plan-your-trip/where-to-stay/masdar-city/masdar-city-hero.jpg',
            ...
            [
                ...
                'image_url' => 'https://www.visitabudhabi.ae/-/media/visitabudhabi/plan-your-trip/where-to-stay/al-maryah-island/al-maryah-island-hero.jpg',
        ...
        foreach ($areas as $areaData) {
            ...
            if ($existingArea) {
                $this->command->info("Area {$areaData['name_en']} already exists, updating image...");
                ...
                try {
                    ...
                    if ($imageContent->successful()) {
                        ...
                        if ($existingArea->main_image && file_exists($areasDir . '/' . $existingArea->main_image)) {
                            unlink($areasDir . '/' . $existingArea->main_image);
                        }
                        
                        $imageName = $areaData['slug'] . '_real_' . time() . '.jpg';
                        $imagePath = $areasDir . '/' . $imageName;
                        
                        file_put_contents($imagePath, $imageContent->body());
                        
                        // Update area with new image
                        $existingArea->update([
                        ...
                        $this->command->info("Updated area: {$areaData['name_en']} with real image: {$imageName}");
                    } else {
                        $this->command->error("Failed to download real image for {$areaData['name_en']}");
                    }
                } catch (\Exception $e) {
                    $this->command->error("Error updating area {$areaData['name_en']}: " . $e->getMessage());
                }
            } else {
                $this->command->info("Area {$areaData['name_en']} not found, skipping...");
            }
        }
        
        $this->command->info('Abu Dhabi Areas Real Images seeder completed successfully!');
    }
}
